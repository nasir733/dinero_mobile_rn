    ...
    return (
        <View style={{flex: 1}}>
            <ScrollView
                ...
                onScroll={(event) =>
                    setDist(event.nativeEvent.contentOffset.y)
                }>
                <Splitter />
                {data.map((dt) => {
                    return (
            ...
            <Animated.View
                ...
                {dist > data.indexOf(active) * headerHeight &&
                    dist <
                        data.indexOf(active) * headerHeight +
                            headerHeight * active.data.length &&
                    JSON.stringify(active) !== JSON.stringify(initialState) && (
                        <View>
                            ...
                            <HeaderMenu
                                data={active}
                                isActive={true}
                                onPress={() => onCategoryPress(active)}
                            />
                        </View>
                    )}
            </Animated.View>
                    ...
                    {data.data.map((dt) => {
                        return (
                            <TouchableOpacity
                                ...
                                onPress={() =>
                                    pageContext.pageDispatch({page: dt.action})
                                }>
...
function HeaderMenu({data, isActive, onPress}) {
    return (
        <TouchableOpacity
            ...
            />
            <Text
                style={{flex: 1, fontSize: 16, color: 'black', marginLeft: 15}}>
                {data.title}
            </Text>
...
function Splitter() {
    return <View style={{height: 0.5, backgroundColor: 'white'}} />;
}
