...
class RangeSlider extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            minValue:
                (this.props.initMinValue * (this.props.sliderSize - 30)) /
                (this.props.maxValue - this.props.minValue),
            maxValue:
                (this.props.initMaxValue * (this.props.sliderSize - 30)) /
                (this.props.maxValue - this.props.minValue),
            minDragValue: 0,
            maxDragValue: 0,
            statusMinDrag: false,
    ...
    _handleMinPanResponderMove = (evt, gestureState) => {
        ...
        if (
            this.state.minValue + dx >= 0 &&
            this.state.minValue + dx + 20 < this.state.maxValue
        ) {
            this.setState({
                statusMinDrag: true,
    ...
    _handleMaxPanResponderMove = (evt, gestureState) => {
        let dx = gestureState.dx;
        if (
            this.state.maxValue + dx <= this.state.layoutWidth &&
            this.state.maxValue + dx - 20 > this.state.minValue
        ) {
            this.setState({
                statusMaxDrag: true,
    ...
    _handleMinResponderRelease = () => {
        this.setState({
            statusMinDrag: false,
            minValue: this.state.minDragValue,
        });
    };
    ...
    _handleMaxResponderRelease = () => {
        this.setState({
            statusMaxDrag: false,
            maxValue: this.state.maxDragValue,
        });
    };

    _handleCallbackValue() {
        let rangeValue = this.props.maxValue - this.props.minValue;
        let minVal = this.state.statusMinDrag
            ? this.state.minDragValue
            : this.state.minValue;
        minVal = Math.round((minVal * rangeValue) / this.state.layoutWidth);
        let maxVal = this.state.statusMaxDrag
            ? this.state.maxDragValue
            : this.state.maxValue;
    ...
    getStyleBarStyle() {
        return [
            {
                position: 'absolute',
                left: this.state.statusMinDrag
                    ? this.state.minDragValue
                    : this.state.minValue,
                width:
                    (this.state.statusMaxDrag
                        ? this.state.maxDragValue
                        : this.state.maxValue) -
                    (this.state.statusMinDrag
                        ? this.state.minDragValue
                        : this.state.minValue) +
                    15,
                height: 5,
    ...
    render() {
        return (
            <View style={[{justifyContent: 'center'}, this.props.style]}>
                ...
                <Animated.View
                    style={this.getStyleMinStyle()}
                    {...this.panResponderMin.panHandlers}>
                    <View
                        style={{
                            width: 16,
                            height: 16,
                ...
                <Animated.View
                    style={this.getStyleMaxStyle()}
                    {...this.panResponderMax.panHandlers}>
                    <View
                        style={{
                            width: 16,
                            height: 16,
