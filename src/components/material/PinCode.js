...
export default class PinCode extends Component {
    ...
    border =
        this.props.withBorder === undefined ? false : this.props.withBorder;
    color = this.props.color === undefined ? '#000' : this.props.color;
    bordercolor =
        this.props.bordercolor === undefined ? '#000' : this.props.bordercolor;
    state = {
        value: '',
        focused: false,
    };
    ...
    handleChange = (value) => {
        ...
        return this.props.onChange(
            (this.state.value + value).slice(0, this.CODE_LENGTH.length),
        );
    };
    ...
    render() {
        ...
        const selectedIndex =
            values.length < this.CODE_LENGTH.length
                ? values.length
                : this.CODE_LENGTH.length - 1;
                ...
                <TouchableWithoutFeedback onPress={this.handleClick}>
                    <View style={styles.wrap}>
                        ...
                        {this.CODE_LENGTH.map((v, index) => {
                            ...
                            const filled =
                                values.length === this.CODE_LENGTH.length &&
                                index === this.CODE_LENGTH.length - 1;

                            return (
                                <View
                                    style={[
                                        this.border
                                            ? styles.display_border
                                            : styles.display,
                                        {
