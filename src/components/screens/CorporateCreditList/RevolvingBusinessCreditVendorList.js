...
export const RevolvingBusinessCreditVendorList = () => {
    const [page, setPage] = useState(0);
    const [data, setData] = useState([]);
    const [isLoading, setLoading] = useState(true);

    useEffect(() => {
        getBusinessData('/business/revolving_credit_vendor/')
            .then((data) => {
                    ...
                    <View style={{flex: 1}}>
                        <ScrollView style={{marginBottom: 10}}>
                            <View style={{flex: 1}}>
                                ...
                                {/*<Text style={dataStyle}>Category: {item.category?item.category:"None"}</Text>*/}
