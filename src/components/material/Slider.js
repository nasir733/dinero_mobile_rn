...
class Slider extends Component {
    ...
    _handleMaxPanResponderMove = (evt, gestureState) => {
        ...
        if (
            this.state.value + dx <= this.state.layoutWidth &&
            this.state.value + dx >= 0
        ) {
            this.setState({
                statusDrag: true,
    ...
    _handleCallbackValue() {
        let maxVal = this.state.statusDrag
            ? this.state.dragValue
            : this.state.value;
